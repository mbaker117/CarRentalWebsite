 ...
 * @author moham
 */
@WebServlet(name = "CustomerOrderServer", urlPatterns = {"/CustomerOrderServer"})
public class CustomerOrderServer extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
          ...
          int customerId = Integer.parseInt(request.getParameter("customerId").trim());
           ...
           int res = CustomerHandler.addOrder(new Order(0, customerId, carId,0, rentDistance, days, additionalFees, 0, false, 0));

                  HttpSession session = request.getSession();
                    session.setAttribute("customerOp", "add");
                    session.setAttribute("res", res);
          

       
       
        response.sendRedirect("/web2/pages/CustomerNewOrderPage.jsp");
    }
