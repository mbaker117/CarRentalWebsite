 ...
 * @author moham
 */
@WebServlet(name = "AdminCarServer", urlPatterns = {"/AdminCarServer"})
public class AdminCarServer extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!op.equals("Delete")) {
            ...
            try {
                if (op.equals("Add")) {
                    int res = AdminHandler.addCar(new Car(0, vin, plateNumber, seats, maker, modelName, modelYear, meterReading, categoryId, isRented, isAvailable));

                    HttpSession session = request.getSession();
                    session.setAttribute("adminCarOp", "add");
                    session.setAttribute("adminCarRes", res);
                } else {
                    int res = AdminHandler.editCar(new Car(id, vin, plateNumber, seats, maker, modelName, modelYear, meterReading, categoryId, isRented, isAvailable));
            ...
            int res = AdminHandler.deleteCar(id);

            HttpSession session = request.getSession();
            session.setAttribute("adminCarOp", "delete");
            session.setAttribute("adminCarRes", res);
        }
        response.sendRedirect("/web2/pages/AdminCarsPage.jsp");
    }
