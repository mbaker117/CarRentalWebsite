 ...
 * @author moham
 */
@WebServlet(name = "EmpOrderServer", urlPatterns = {"/EmpOrderServer"})
public class EmpOrderServer extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!op.equals("Delete") && !op.equals("Check")) {
            ...
            if (op.equals("Add")) {
                 int res = EmployeeHandler.addOrder(new Order(0, customerId, carId,0, rentDistance, days, additionalFees, 0, true, 0));
                ...
                float payment = Float.parseFloat(request.getParameter("payment").trim());
                 int res = EmployeeHandler.editOrder(new Order(id, customerId, carId,0, rentDistance, days, additionalFees, 0, true, payment));
                    HttpSession session = request.getSession();
            ...
            if(op.equals("Delete")){
            int res = EmployeeHandler.deleteOrder(id);

            HttpSession session = request.getSession();
            session.setAttribute("adminOrderOp", "delete");
            session.setAttribute("adminOrderRes", res);
            }else{
                int res = EmployeeHandler.checkOut(id);
        ...
        response.sendRedirect("/web2/pages/EmpOrderPage.jsp");
    }
